...
"""
    ...
    ARCH_INFO_FIELDS = """
    info(aa_policy, "{arch_policy}")
    info(archiver, "{arch_sys}")"""
...
""".format(
            ...
            plcf_branch=keyword_params.get('BRANCH', "N/A"),
            plcf_branch_39=keyword_params.get('BRANCH', "N/A")[:39],
            timestamp=keyword_params.get('TIMESTAMP'), 
            cmdline=keyword_params.get('CMD_LINE', "N/A"),
            modversion=keyword_params.get('DEF_MODVERSION', 'N/A'),
        )
...
""".format(
    ...
    def _body_var(self, var, output, **keyword_params):
        ...
        self._append(self._toEPICS(var, **keyword_params))
...
class EPICS(EPICS_BASE):
    ...
    def _toEPICS(self, var, **keyword_params):
        pv_extra = (
        ...
        arch = var.get_parameter("ARCH_POL", False)
        #print(arch)
        arch_sys = keyword_params.get("ARCH_CLTR", False)
        #print(arch_sys)
        if arch and arch_sys:
            if isinstance(arch, bool):
                pv_extra += EPICS_BASE.ARCH_INFO_FIELDS.format(
                arch_policy="default",
                arch_sys=arch_sys)
            else:
                pv_extra += EPICS_BASE.ARCH_INFO_FIELDS.format(
...
"""
        ...
        for src in if_def.interfaces():
            if isinstance(src, BLOCK):
                ...
                self._body_var(src, output, **keyword_params)
                #if src.get_parameter("ARCHIVE", False):
                #    print(src.fqpn(), src.get_parameter("ARCHIVE"))
            elif isinstance(src, VERBATIM):
                self._append((src.source(), str(src)))
            elif isinstance(src, SOURCE):
                self._body_source(src, output)
            else:
                self._append(self._toEPICS(src, **keyword_params))
